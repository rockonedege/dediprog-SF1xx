...
// -anderson_variable 06/10/11 ///////////////////////////////////////
    ...
    do //for(unsigned int idx = 0; idx < iULBA; ++idx)
    {
        ...
        while(len > 0)
        {
            ...
            std::copy(vBuffer.begin() + one_record.intel_adr + (idx<<16), 
...
bool HexFileToBin(const string& filePath, vector<unsigned char>& vOutData)
{
    ...
    /* allocate a buffer */
    vector<unsigned char> Memory_Block(MEMORY_SIZE, PadByte) ;
...
//     fprintf(stdout,"Pad Byte        = %X\n",  PadByte);
    ...
    */
    ...
    {
        copy(Memory_Block.begin() + Lowest_Address, Memory_Block.begin() + Highest_Address + 1, std::back_inserter(vOutData)) ;
    }
